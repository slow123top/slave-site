...
router.post('/', function (req, res, next) {
    ...
    let allPath = new path.allPath(platform('1'), 'tgas.dat').getPath()
    ...
    readLine.on('line', line => {
        ...
        let lineRadeg = Number(line.substring(75, 89))
        let lineDedeg = Number(line.substring(98, 111))
        let r = Math.acos(Math.sin(lineDedeg) * Math.sin(de) + Math.cos(lineDedeg) * Math.cos(ra) * Math.cos(lineRadeg - ra));
        if (Number(r) < radius) {
